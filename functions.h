...
void trianglesShading()
{
	...
	else if (shading_type == 1) //flat shading without phong term
	{
		for (int i = 0; i < triangles_list.size(); i++) //triangle index
		{
			Vertex vec_1 = vertice_list[triangles_list[i].v_idx[1]] - vertice_list[triangles_list[i].v_idx[0]];
			Vertex vec_2 = vertice_list[triangles_list[i].v_idx[2]] - vertice_list[triangles_list[i].v_idx[0]];
			Vertex n = vec_1.cross(vec_2);
			n.onBtnPressize();
			...
			Vertex r;
			r.x = n.uni_x * 2.0f * cos_theta - l.uni_x;
			r.y = n.uni_y * 2.0f * cos_theta - l.uni_y;
			r.z = n.uni_z * 2.0f * cos_theta - l.uni_z;
			r.onBtnPressize();
			float cos_alpha = v.normDot(r);
			...
			triangles_list[i].c[0].channel[0] = ambient_color.channel[0] * bunny_color.channel[0] + light.c.channel[0] * bunny_color.channel[0] * cos_theta;
			triangles_list[i].c[0].channel[1] = ambient_color.channel[1] * bunny_color.channel[1] + light.c.channel[1] * bunny_color.channel[1] * cos_theta;
			triangles_list[i].c[0].channel[2] = ambient_color.channel[2] * bunny_color.channel[2] + light.c.channel[2] * bunny_color.channel[2] * cos_theta;
		}
	...
	else if (shading_type == 2) //flat shading with phong term
	{
		...
		for (int i = 0; i < triangles_list.size(); i++) //triangle index
		{
			...
			triangles_list[i].c[1].channel[0] += phong_color.channel[0] * light.c.channel[0] * powf(cos_alpha, PHONG);
			triangles_list[i].c[1].channel[1] += phong_color.channel[1] * light.c.channel[1] * powf(cos_alpha, PHONG);
			triangles_list[i].c[1].channel[2] += phong_color.channel[2] * light.c.channel[2] * powf(cos_alpha, PHONG);
...
void initialize()
{
	...
	phong_color.channel[0] = 0.5;
	phong_color.channel[1] = 0.5;
